...
try:
    import cv2
except ImportError:  # optional — only needed for heightfield helpers
    cv2 = None
try:
    import noise
except ImportError:
...
# Adapted for this repo from unitree_mujoco/terrain_tool.
# Prefer scripts/generate_stairs_ledges.py for Go2 stair/ledge worlds.
from pathlib import Path

_HERE = Path(__file__).resolve().parent
_ENVS = _HERE.parent / "envs"

ROBOT = "go2"
INPUT_SCENE_PATH = str(_HERE / "base_scene.xml")
OUTPUT_SCENE_PATH = str(_ENVS / "go2_unitree_terrain_generated.xml")
HFIELD_DIR = _HERE / "generated"
HFIELD_DIR.mkdir(parents=True, exist_ok=True)
...
class TerrainGenerator:
    ...
    def AddPerlinHeighField(
        ...
        if noise is None or cv2 is None:
            raise ImportError("AddPerlinHeighField needs: pip install noise opencv-python")
        ...
        out_path = HFIELD_DIR / output_hfield_image
        cv2.imwrite(str(out_path), terrain_image)
        ...
        hfield.attrib["file"] = str(out_path)
    ...
    def AddHeighFieldFromImage(
        ...
        if cv2 is None:
            raise ImportError("AddHeighFieldFromImage needs: pip install opencv-python")
